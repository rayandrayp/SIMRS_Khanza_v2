        ...
        if(tabMode.getRowCount()==0){
            ...
            if(akses.getSoftDeletes()) {
                Sequel.simpanDelete("INSERT INTO deleted_data (nama_tabel, deleted_at, deleted_by, data_col1, data_col2, data_col3, data_col4, data_col5, data_col6, data_col7, data_col8) "
                    + "SELECT 'resep_obat',NOW(),'"+akses.getkode()+"', no_resep, tgl_perawatan, jam, no_rawat, kd_dokter, tgl_peresepan, jam_peresepan, status FROM resep_obat WHERE no_resep = '"+NoResep.getText()+"'");
            }
...
//            }
            ...
            if(Sequel.cariInteger(
                ...
                "resep_obat.jam=aturan_pakai.jam where resep_obat.no_rawat=? and aturan_pakai.aturan<>''",TNoRw.getText())>0){

                //Edited by Rayandra on 11/01/2022
                Valid.MyReportqry("rptItemResep.jasper","report","::[ Aturan Pakai Obat ]::",
                    ...
                    "aturan_pakai.aturan, " +
                    "detail_pemberian_obat.jumlahawal as jml, " +
                    "kodesatuan.satuan, " +
                    "pasien.jk, " +
                    ...
                    "pasien.no_ktp, " +
                    "bangsal.nm_bangsal, " +
                    "IFNULL(databarang.expire, 0) as exp_date " +
                    "from " +
                    "resep_obat " +
                    ...
                    "inner join bangsal on resep_obat.no_rawat = reg_periksa.no_rawat " +
...
//                    "where resep_obat.no_resep='"+NoResep.getText()+"' and aturan_pakai.aturan<>''",param);
            ...
            if(Sequel.cariInteger(
                ...
                Valid.MyReportqry("rptItemResep2.jasper","report","::[ Aturan Pakai Obat ]::",
                    ...
                    "where resep_obat.no_resep='"+NoResep.getText()+"'  GROUP BY resep_obat.no_resep",param);
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ps=koneksi.prepareStatement(
                    "select databarang.nama_brng,master_aturan_pakai.signa,detail_pemberian_obat.jml,kodesatuan.satuan, detail_pemberian_obat.jumlahawal "+
                    "from resep_obat inner join reg_periksa inner join "+
                    "aturan_pakai inner join databarang inner join detail_pemberian_obat "+
                    "inner join kodesatuan inner join master_aturan_pakai on resep_obat.no_rawat=reg_periksa.no_rawat  "+
                    ...
                    "resep_obat.jam=aturan_pakai.jam and resep_obat.no_rawat=detail_pemberian_obat.no_rawat "+
                    "and resep_obat.tgl_perawatan=detail_pemberian_obat.tgl_perawatan and " +
                    "resep_obat.jam=detail_pemberian_obat.jam and kodesatuan.kode_sat=databarang.kode_sat and aturan_pakai.aturan = master_aturan_pakai.aturan "+
...
//                    System.out.println(ps.toString());
                    ...
                    while(rs.next()){
                        Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                            "0",rs.getString("nama_brng"),rs.getString("signa"),intToRoman(rs.getInt("jumlahawal")),rs.getString("satuan"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
                        });
                    }
                } catch (Exception e) {
                ...
                psracikan=koneksi.prepareStatement(
                    ...
                    "obat_racikan.no_rawat,obat_racikan.aturan_pakai,obat_racikan.jml_dr,metode_racik.nm_racik,metode_racik.signa as mc_signa,master_aturan_pakai.signa as map_signa " +
                    "from resep_obat inner join reg_periksa inner join " +
                    "obat_racikan inner join metode_racik INNER JOIN master_aturan_pakai on resep_obat.no_rawat=reg_periksa.no_rawat " +
                    "and obat_racikan.kd_racik=metode_racik.kd_racik " +
                    "and resep_obat.no_rawat=obat_racikan.no_rawat and " +
                    "resep_obat.tgl_perawatan=obat_racikan.tgl_perawatan and " +
                    "resep_obat.jam=obat_racikan.jam and resep_obat.no_rawat=obat_racikan.no_rawat AND obat_racikan.aturan_pakai = master_aturan_pakai.aturan "+
                ...
                try {
                    ...
                    while(rsracikan.next()){
                        ...
                        try {
                            ...
                            total=0;
                            while(rs2.next()){
                                rincianobat="     "+rs2.getString("nama_brng")+""+rincianobat;
...
//                        Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
//                            "0",rs2.getString("mc_signa"),rsracikan.getString("map_signa"),intToRoman(rsracikan.getInt("jml_dr")),rsracikan.getString("nm_racik"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
//                        });
                        Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                            "0",rsracikan.getString("nama_racik")+" "+rincianobat+"",rsracikan.getString("mc_signa")+" "+rsracikan.getString("map_signa")+"",intToRoman(rsracikan.getInt("jml_dr")),rsracikan.getString("nm_racik"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
                        });
//                        Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
//                            "0",rsracikan.getString("nama_racik")+" ("+rincianobat+")",rsracikan.getString("aturan_pakai"),intToRoman(rsracikan.getInt("jml_dr")),rsracikan.getString("nm_racik"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
//                        });
                    }
                } catch (Exception e) {
            ...
            String alergi_ya,alergi_tidak,alergi_obat;
            alergi_obat = Sequel.cariIsi("SELECT alergi FROM penilaian_awal_keperawatan_ralan WHERE no_rawat IN (SELECT no_rawat FROM reg_periksa WHERE no_rkm_medis = (SELECT no_rkm_medis FROM reg_periksa WHERE no_rawat = ?)) ORDER BY tanggal DESC LIMIT 1",TNoRw.getText());
            if(alergi_obat.contains("-") || alergi_obat.contains("tidak") || alergi_obat.contains("")){
                alergi_ya = "";
                alergi_tidak = "V";
                alergi_obat = "";
            } else {
            ...
            Map<String, Object> param = new HashMap<>(); 
            param.put("jnsResep",Sequel.cariIsi("select png_jawab from penjab where kd_pj=?",Sequel.cariIsi("select kd_pj from reg_periksa where no_rawat=?",TNoRw.getText())));
            param.put("jam_terima",Sequel.cariIsi("select ifnull(temp5,'-') from temporary2 where temp2=?",TNoRw.getText())); 
            param.put("no_sip",Sequel.cariIsi("select no_ijn_praktek from dokter where nm_dokter=?",NmDokter.getText())); 
            param.put("no_hp",Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?",TNoRm.getText())); 
            ...
            param.put("status_perawatan",Sequel.cariIsi("select status_lanjut from reg_periksa where no_rawat=?",TNoRw.getText())); 
            param.put("diagnosa",Sequel.cariIsi("SELECT CONCAT(kd_penyakit,'-',nm_penyakit) AS diagnosa FROM penyakit WHERE kd_penyakit = (SELECT kd_penyakit FROM diagnosa_pasien WHERE no_rawat = ?);", TNoRw.getText())); 
            param.put("tb_bb",Sequel.cariIsi("SELECT CONCAT(TB,'/ ', BB) FROM penilaian_awal_keperawatan_ralan WHERE no_rawat IN (SELECT no_rawat FROM reg_periksa WHERE no_rkm_medis = (SELECT no_rkm_medis FROM reg_periksa WHERE no_rawat = ?)) ORDER BY tanggal DESC LIMIT 1",TNoRw.getText()));
            param.put("alergi_ya",alergi_ya);
            param.put("alergi_tidak",alergi_tidak);
            param.put("alergi_obat",alergi_obat);
            param.put("namars",akses.getnamars());
            param.put("alamatrs",akses.getalamatrs());
            param.put("kotars",akses.getkabupatenrs());
            param.put("propinsirs",akses.getpropinsirs());
            ...
            param.put("finger","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+" Ditandatangani secara elektronik oleh "+NmDokter.getText()+" ID "+(finger.equals("")?KdDokter.getText():finger)+" "+DTPBeri.getSelectedItem());  
...
//                    System.out.println("ps "+ps);
                    ...
                    System.out.println("<html><body><center><img src='https://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/penyerahanresep/"+rs.getString("photo")+"' alt='photo' width='550' height='500'/></center></body></html>");
                } catch (Exception e) {
                    System.out.println("Notif : "+e);
                } finally{
                    if(rs!=null){
